...
import { validateNotFuture, getCurrentDateTimeLocal } from '../../lib/dateValidation';
...
export function MealInput({ onSubmit, isLoading, error }: MealInputProps) {
  ...
  const handleFormSubmit = (data: CreateMealInput) => {
    ...
    // Validate that the date is not in the future (FR-005)
    const validationError = validateNotFuture(recordedAt || new Date().toISOString());
    if (validationError) {
